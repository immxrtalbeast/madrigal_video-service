...
- `POST /videos/{id}/draft:approve` — принять (или отредактировать) сториборд, после чего пайплайн продолжает работу (ассеты → озвучка → рендер).
...
- `POST /media` — загрузка пользовательских изображений/видео (base64) прямо в бакет; `GET /media?folder=assets/test` — список доступных файлов в папке (для выбора фона).
