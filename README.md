...
- `POST /media` — загрузка пользовательских изображений/видео (base64) прямо в бакет; `GET /media?folder=backgrounds` вернёт объединённый список изображений: сначала ваши файлы из `assets/{user_id}/…`, затем общий каталог (shared). Папки и 0‑байтовые служебные объекты фильтруются по расширению, поэтому «assets/.../images» в ответ не появятся. Отдельные запросы к `/media/shared` оставлены для совместимости, но фронту достаточно одного вызова.
