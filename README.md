...
- `POST /media/videos`, `GET /media/videos`, `GET /media/shared/videos` — отдельные ручки для загрузки/листа фоновых видеороликов (mp4/webm и т.п.), чтобы затем подставлять их в качестве единственного фона вместо набора картинок. Дополнительно есть `POST /media/videos:upload`, принимающий `multipart/form-data` (`folder`, опциональный `filename`, поле `file=@clip.mp4`) — можно грузить крупные ролики без base64.
...
### Фоновые видео
- `POST /media/videos` / `GET /media/videos` / `GET /media/shared/videos` — загрузка и выбор видеороликов (mp4/webm/mov), которые можно использовать в качестве единственного фона на протяжении всего финального ролика.
