...
- `POST /media/videos`, `GET /media/videos`, `GET /media/shared/videos` — отдельные ручки для загрузки/листа фоновых видеороликов (mp4/webm и т.п.), чтобы затем подставлять их в качестве единственного фона вместо набора картинок.
